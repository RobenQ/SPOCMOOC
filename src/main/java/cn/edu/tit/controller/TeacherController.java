...
public class TeacherController {
	 ...
	 * @return
	 */
	@RequestMapping({"toCourseDetail/{courseId}"})
	  public String toCourseDetail(HttpServletRequest request, @PathVariable String courseId) throws Exception {
	    Course course = null;
	    try {
	      course = this.teacherService.getCourseById(courseId);
	    ...
	    List<Teacher> teacherList = this.teacherService.getTeachersByCourseId(courseId);
	    request.getSession().setAttribute("teacherList", teacherList);
	    
	    Teacher teacher = (Teacher)request.getSession().getAttribute("teacher");
	    if (teacher != null) {
	      
	      Integer manager = this.teacherService.getManagerByEmployeeNum(teacher.getEmployeeNum(), courseId, 1);
	      request.setAttribute("manager", manager);
	      
	      Integer attention = this.teacherService.getManagerByEmployeeNum(teacher.getEmployeeNum(), courseId, 2);
	    ...
	    if (student != null) {
	      ...
	      VirtualClass virtualClass = this.teacherService.getVirtualClassByRidAndCid(student.getClassNum(), courseId);
	      request.setAttribute("virtualClass", virtualClass);
	      
	      Integer attention = this.studentService.getManagerByStudentId(student.getStudentId(), courseId, 2);
	      request.setAttribute("attention", attention);
	    } 
	    request.setAttribute("student", student);
	    
	    String category = this.teacherService.getCategoryById(course.getCourseCategory());
	    ...
	    try {
	      List<Task> teaching_plan = this.teacherService.getTaskByPointAndCourse("teaching_plan", courseId);
	      int teaching_planNum = teaching_plan.size();
	      request.setAttribute("teaching_planNum", Integer.valueOf(teaching_planNum));
	      
	      List<Task> teaching_resourse = this.teacherService.getTaskByPointAndCourse("teaching_resourse", courseId);
	      int teaching_resourseNum = teaching_resourse.size();
	      request.setAttribute("teaching_resourseNum", Integer.valueOf(teaching_resourseNum));
	      
	      List<Resource> mediaRList = this.resourceService.showResourceByCourse(courseId);
	      int mediaRListNum = mediaRList.size();
	      request.setAttribute("mediaRListNum", Integer.valueOf(mediaRListNum));
	      for (Resource resource : mediaRList) {
	        resource.setPublisherId(this.teacherService.getTeacherNameById(resource.getPublisherId()));
	      }
	      if (!mediaRList.isEmpty()) {
	        request.setAttribute("resource", mediaRList);
	      }
	      
	      List<Task> taskList = this.teacherService.getTaskByPointAndCourse("work", courseId);
	      int taskListNum = taskList.size();
	      request.setAttribute("taskListNum", Integer.valueOf(taskListNum));
	      for (Task task : taskList) {
	        task.setPublisherId(this.teacherService.getTeacherNameById(task.getPublisherId()));
	      }
	      if (!taskList.isEmpty()) {
	        request.setAttribute("taskList", taskList);
	      }
	      
	      List<Task> trialList = this.teacherService.getTaskByPointAndCourse("trial", courseId);
	      int trialListNum = trialList.size();
	      request.setAttribute("trialListNum", Integer.valueOf(trialListNum));
	      for (Task task : trialList) {
	        task.setPublisherId(this.teacherService.getTeacherNameById(task.getPublisherId()));
	      }
	      if (!trialList.isEmpty()) {
	        request.setAttribute("taskList", trialList);
	      }
	      
	      List<Task> curriculum_design = this.teacherService.getTaskByPointAndCourse("course_design", courseId);
	      int curriculum_designNum = curriculum_design.size();
	      request.setAttribute("curriculum_designNum", Integer.valueOf(curriculum_designNum));
	      
	      List<CourseExpand> aocscList = this.iAchievementService.queryCourseExpandByCourseId(courseId);
	      int aocscListNum = aocscList.size();
	      request.setAttribute("aocscListNum", Integer.valueOf(aocscListNum));
