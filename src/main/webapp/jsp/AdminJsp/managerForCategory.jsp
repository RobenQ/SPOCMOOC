	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_departments')
					.bootstrapTable(
							{
								url : '${pageContext.request.contextPath}/admin/getCategoryInfo', //请求后台的URL（*）
								method : 'get', //请求方式（*）
										...
										},
										{
											...
											events : {
												'click #edit' : function(event,
														value, row, index) {
													var id = row.id;
													var categoryNum = row.categoryNum;
													var name = row.categoryName;
													var categoryDetail = row.categoryDetail;
													document
															.getElementById("editCategoryNum").value = categoryNum;
													document
															.getElementById("categoryId").value = id;
													document
															.getElementById("editCategoryName").value = name;
													document
															.getElementById("editCategorDetail").value = categoryDetail;
													$('#Edit').modal('show');
												}
											}
										} ],
	...
	<div class="content">
		...
		<div class="RightContent col-md-10" style="margin-top: 1%;">
			<div class="panel-body"
				...
				<h2 class="text-center">课程类型管理</h2>
				<hr style="margin: 1%">
				<button type="button" class="btn btn-primary btn-sm"
					data-toggle="modal" data-target="#myModal" style="margin-top: 1%">添加分类</button>
